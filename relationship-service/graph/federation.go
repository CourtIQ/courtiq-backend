...
func (ec *executionContext) resolveEntity(
	...
	switch typeName {
	case "Coachship":
		resolverName, err := entityResolverNameForCoachship(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Coachship": %w`, err)
		}
		switch resolverName {
		...
		case "findCoachshipByID":
			...
			entity, err := ec.resolvers.Entity().FindCoachshipByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Coachship": %w`, err)
			}

			return entity, nil
		}
	case "Friendship":
		resolverName, err := entityResolverNameForFriendship(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Friendship": %w`, err)
		}
		switch resolverName {
		...
		case "findFriendshipByID":
			...
			entity, err := ec.resolvers.Entity().FindFriendshipByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Friendship": %w`, err)
			}
	...
	case "Relationship":
		...
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Relationship": %w`, err)
		}
		switch resolverName {
		...
		case "findRelationshipByID":
			...
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Relationship": %w`, err)
			}
...
func entityResolverNameForCoachship(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Coachship", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Coachship", ErrTypeNotFound))
			break
		}
		return "findCoachshipByID", nil
	}
	return "", fmt.Errorf("%w for Coachship due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}

func entityResolverNameForFriendship(ctx context.Context, rep EntityRepresentation) (string, error) {
	// we collect errors because a later entity resolver may work fine
	// when an entity has multiple keys
	entityResolverErrs := []error{}
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Friendship", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Friendship", ErrTypeNotFound))
			break
		}
		return "findFriendshipByID", nil
	}
	return "", fmt.Errorf("%w for Friendship due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}

func entityResolverNameForRelationship(ctx context.Context, rep EntityRepresentation) (string, error) {
	// we collect errors because a later entity resolver may work fine
	// when an entity has multiple keys
	entityResolverErrs := []error{}
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Relationship", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Relationship", ErrTypeNotFound))
			break
		}
		return "findRelationshipByID", nil
	}
	return "", fmt.Errorf("%w for Relationship due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}
