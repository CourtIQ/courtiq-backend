...
func (ec *executionContext) resolveEntity(
	...
	switch typeName {
	case "StringEntry":
		resolverName, err := entityResolverNameForStringEntry(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "StringEntry": %w`, err)
		}
		switch resolverName {
		...
		case "findStringEntryByID":
			...
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findStringEntryByID(): %w`, err)
			}
			entity, err := ec.resolvers.Entity().FindStringEntryByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "StringEntry": %w`, err)
			}
...
func entityResolverNameForStringEntry(ctx context.Context, rep EntityRepresentation) (string, error) {
	for {
		var (
		...
		return "findStringEntryByID", nil
	}
	return "", fmt.Errorf("%w for StringEntry", ErrTypeNotFound)
}
